...
**Project:** Opscale Actions
**Project Type:** package
**Module Prefix:** actions
**Tenant Aware:** no
**Created:** 2026-05-01
...
**Package note:** `opscale-co/actions` is itself the framework that provides the base
`Action` class, decorators, adapters, and design-pattern wrappers used by every consuming
Opscale project. Internal classes in `src/` extend or wrap `lorisleiva/laravel-actions`
infrastructure — they are not subject to the "one Action per BPMN task" rule, since this
package has no BPMN. Consumers of the package follow that rule in their own projects.
...
This package does not produce Outputs — it provides the Action infrastructure consumers
use to dispatch their own Outputs.
...
This package does not require tenant isolation.

**Reason:** `opscale-co/actions` is a standalone Laravel/Nova extension package that
provides reusable Action infrastructure (decorators, adapters, design-pattern wrappers)
on top of `lorisleiva/laravel-actions`. It carries no domain tables of its own — the
consuming application owns all tenant scoping. Adding `tenant_id` columns or
Repository-level tenant filters would create false coupling between this package and
any specific multi-tenancy strategy.

If a consumer application is tenant-aware, tenant scoping is enforced in that
application's Repositories and Nova Resources, not here.
